...
function Brand() {
  return (
    <Link href="/home" className="flex items-center gap-2.5 px-1">
      <div className="grid h-8 w-8 place-items-center rounded-[5px] bg-gradient-to-br from-[#fe772c] to-[#0a2152] text-white shadow-[0_0_18px_-4px_rgba(254,119,44,0.5)]">
            ...
            <div key={section.group}>
              ...
              <div className="flex flex-col gap-0.5">
                {items.map((item) => {
                  ...
                  return (
                    <Link key={item.href} href={item.href} onClick={onNavigate} className={cn("group relative flex items-center gap-3 rounded-[5px] px-3 py-2 text-sm font-medium transition-colors", active ? "text-chrome-text" : "text-chrome-text-faint hover:text-chrome-text")}>
                      {active && <motion.span layoutId="nav-active" className="absolute inset-0 rounded-[5px] border border-[rgba(254,119,44,0.3)] bg-[rgba(254,119,44,0.14)]" transition={{ type: "spring", stiffness: 400, damping: 32 }} />}
...
function SidebarInner({ onNavigate }: { onNavigate?: () => void }) {
  return (
    <div className="flex h-full flex-col gap-6 p-4">
      ...
      <div className="rounded-[6px] border border-chrome-border bg-chrome-2 p-3">
...
function ThemeToggle() {
  ...
  return (
    <button aria-label="Toggle theme" onClick={toggle} className="grid h-9 w-9 place-items-center rounded-[5px] border border-border bg-surface text-text-muted transition-colors hover:text-text">
...
function ClientSwitcher() {
  ...
  if (!isSuper) {
    return (
      <span className="hidden h-9 items-center gap-2 rounded-[5px] border border-border bg-surface px-3 md:flex">
  ...
  return (
    <div className="relative hidden h-9 items-center rounded-[5px] border border-border bg-surface md:flex">
...
function PersonaSwitcher() {
  ...
  return (
    <div className="relative flex h-9 items-center gap-2 rounded-[5px] border border-border bg-surface pl-1 pr-7">
...
function SignOut() {
  ...
  return (
    <button
      ...
      className="grid h-9 w-9 place-items-center rounded-[5px] border border-border bg-surface text-text-muted transition-colors hover:border-negative/40 hover:text-negative"
      aria-label="Sign out"
      title="Sign out"
    >
      <LogOut size={16} />
  ...
  return (
    <>
      <header className="sticky top-0 z-30 flex h-16 items-center gap-2 border-b border-border bg-[color-mix(in_oklab,var(--bg)_82%,transparent)] px-4 backdrop-blur-xl md:gap-3 md:px-6">
        <button onClick={onMenu} className="grid h-9 w-9 shrink-0 place-items-center rounded-[5px] border border-border text-text-muted lg:hidden" aria-label="Open menu"><Menu size={18} /></button>

        <TopbarSearch />

        <div className="ml-auto flex items-center gap-2 md:gap-3">
          <ClientSwitcher />
          <button onClick={() => window.dispatchEvent(new Event("relos-open-copilot"))} className="hidden h-9 items-center gap-1.5 rounded-[5px] border border-accent/40 bg-accent-soft px-3 text-sm font-semibold text-accent transition-colors hover:bg-accent-soft/70 sm:flex" aria-label="Open Agent Copilot">
            <Bot size={16} /> <span className="hidden md:inline">Copilot</span>
          </button>
          <button onClick={() => setFeedOpen(true)} className="relative grid h-9 w-9 place-items-center rounded-[5px] border border-border bg-surface text-text-muted transition-colors hover:text-text" aria-label="Activity feed">
            <Activity size={16} />
        ...
        {drawer && (
          <>
            ...
            <motion.aside className="chrome fixed inset-y-0 left-0 z-50 w-[270px] border-r border-chrome-border lg:hidden" initial={{ x: "-100%" }} animate={{ x: 0 }} exit={{ x: "-100%" }} transition={{ type: "spring", stiffness: 400, damping: 38 }}>
              <button onClick={() => setDrawer(false)} className="absolute right-3 top-4 grid h-8 w-8 place-items-center rounded-[6px] text-chrome-text-faint hover:text-chrome-text" aria-label="Close menu"><X size={18} /></button>
