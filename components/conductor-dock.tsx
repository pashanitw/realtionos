...
export function ConductorDock() {
  ...
  return (
    <>
      <button
        onClick={() => setOpen((o) => !o)}
        className="group fixed bottom-5 left-5 z-[60] hidden h-11 items-center gap-2 rounded-pill border border-chrome-border bg-chrome px-3.5 text-chrome-text shadow-[var(--shadow-lift)] transition-transform hover:scale-[1.03] lg:flex"
        aria-label="Demo conductor"
      >
        <Clapperboard size={16} className="text-[#43c9b8]" />
        <span className="text-xs font-semibold">Conductor</span>
        <kbd className="rounded bg-[rgba(255,255,255,0.08)] px-1 font-mono text-[10px]">⌘J</kbd>
      </button>
      ...
      <AnimatePresence>
        {open && (
          <motion.div
            ...
            className="chrome fixed bottom-20 left-5 z-[60] w-[300px] overflow-hidden rounded-[16px] border border-chrome-border shadow-[var(--shadow-lift)]"
          >
            <div className="flex items-center justify-between border-b border-chrome-border px-4 py-3">
              <div className="flex items-center gap-2">
                <Radio size={15} className="text-[#43c9b8]" />
                <span className="font-display text-sm font-bold text-white">Demo conductor</span>
              </div>
              <button onClick={() => setOpen(false)} className="text-chrome-text-faint hover:text-white"><X size={16} /></button>
            </div>
            <div className="space-y-1 p-2">
              {BEATS.map((b) => (
                <button
                  ...
                  className="flex w-full items-center gap-3 rounded-[10px] px-2.5 py-2.5 text-left transition-colors hover:bg-[rgba(255,255,255,0.05)]"
                >
                  <span className="grid h-8 w-8 place-items-center rounded-lg bg-[rgba(52,179,163,0.14)] text-[#43c9b8]"><b.icon size={15} /></span>
